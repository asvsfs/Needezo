...
export default class MainPage extends Component {
	...
	loadList(list) {
		...
		for (var i = 0 ; i < list.length ;  i++){
			childItem = <ItemBox loading={self.loading.bind(self)} marginTop={0} marginLeft={0} data={list[i]} key={i}/>;
	...
	fetchItemsPage(category) {
		this.loading(true);

		var self = this;
		http
      .post("/fetchItems/"+this.state.page)
      .send({location:{x:window.localStorage.getItem("locationX"),y:window.localStorage.getItem("locationY")},category:category})
      .accept('application/json')
      .end(function(err, res){
      	self.loading(false);
        if(err){
